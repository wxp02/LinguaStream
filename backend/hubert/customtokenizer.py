...
class CustomTokenizer(nn.Module):
    ...
    def train_step(self, x_train, y_train, log_loss=False):
        ...
        y_train_hot = y_train_hot.to('cpu')
...
def auto_train(data_path, save_path='model.pth', load_model: str | None = None, save_epochs=1):
    ...
    if load_model and os.path.isfile(load_model):
        ...
        model_training = CustomTokenizer.load_from_checkpoint(load_model, 'gpu')
    else:
        print('Creating new model.')
        model_training = CustomTokenizer(version=1).to('cpu')
        ...
        for i in range(save_epochs):
            ...
            for i in range(max(len(data_x), len(data_y))):
                ...
                if x is None or y is None:
                    ...
                    continue
                model_training.train_step(torch.tensor(x).to('cpu'), torch.tensor(y).to(device), j % 50 == 0)  # Print loss every 50 steps
                j += 1
        save_p = save_path
